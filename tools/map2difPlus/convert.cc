...
void getBrushes(InteriorMap* map)
{
   ...
   for (U32 i = 0; i < map->mInteriorRes->mBrushes.size(); i++)
   {
      ...
      // Set the initial lighting scale to match the worldspawn lighting scale (this can be overridden by a sgLightingScaleEntity
      pBrush->sgLightingScale = gWorkingGeometry->mWorldEntity->mLumelScale;
...
void getEntities(InteriorMap* map)
{
   for (U32 i = 0; i < map->mInteriorRes->mEntities.size(); i++)
   {
      ...
      // lighting: emitters
      else if (dStricmp(ent->classname, PointEmitterEntity::getClassName()) == 0)
      ...
      // Ignore the worldspawn entry, if it gets in here. We handle it elsewhere.
      else if (dStricmp(ent->classname, WorldSpawnEntity::getClassName()) == 0)
         pEntity = NULL;
