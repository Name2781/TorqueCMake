...
#include "map2difPlus/createLightmaps.h"
...
bool Lighting::SVNode::clipWindingToPlaneFront(U32 planeEQIndex)
{
   ...
   while (end != baseStart) {
      ...
      switch (code) {
        ...
        case 2: { // f b
            ...
            while (rClipPlane.whichSideLow(getPoint(mWinding->mIndices[endSeek])) == PlaneBack)
               endSeek = (endSeek + 1) % mWinding->mNumIndices;

            PlaneSide esSide = rClipPlane.whichSideLow(getPoint(mWinding->mIndices[endSeek]));
   ...
   // Emit the last point.
   AssertFatal(rClipPlane.whichSideLow(getPoint(mWinding->mIndices[start])) != PlaneBack,
